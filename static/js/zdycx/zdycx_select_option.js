...
var cities=new Array(13);//创建一个二维数组存放地市,区县
    ...
    cities[11]=new Array("鹤城区","洪江市","洪江区","会同县","沅陵县","辰溪县","溆浦县","中方县","新晃县","芷江县","通道县","麻阳县","靖州县");
    cities[12]=new Array("娄星区","冷水江市","涟源市","新化县","双峰县");
    cities[13]=new Array("吉首市","龙山县","古丈县","永顺县","凤凰县","保靖县","花垣县");

//设置点击事件
    function changeCity(val){
        district_all.style.display = "block";
        district_select.innerHTML = "";//清空内容
        for(var i=0;i<cities.length;i++){
            //2.创建城市的文本节点
            if(val==i) {
               ////3.遍历用户选择的省份下的城市
                for (var j=0;j<cities[i].length;j++){
                     district_li=document.createElement('li');
                     district_li.className = "condition_level_4";
                     var checkBox=document.createElement("input");
                         checkBox.setAttribute("type","checkbox");
                         checkBox.setAttribute("name", "district");
                         checkBox.setAttribute("value",cities[i][j]);
                         textNode = document.createTextNode(cities[i][j]);
                         district_li.appendChild(checkBox);
                         district_li.appendChild(textNode);
                         district_select.appendChild(district_li);
                         }
                }
            }
        }